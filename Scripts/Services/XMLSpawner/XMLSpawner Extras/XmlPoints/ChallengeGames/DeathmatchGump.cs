...
namespace Server.Gumps
{
	public class DeathmatchGump : Gump
	{
		...
		private DeathmatchGauntlet m_gauntlet;
		...
		public DeathmatchGump( DeathmatchGauntlet gauntlet, Mobile from, int page ) : base( 20, 30 )
		{
			...
			if(gauntlet.Participants != null)
			{
                ...
                for(int i = 0;i<m_WorkingList.Count;i++)
                {
                    ...
                    DeathmatchGauntlet.ChallengeEntry entry = (DeathmatchGauntlet.ChallengeEntry)m_WorkingList[i];
			...
			if(from == gauntlet.Challenger && !m_gauntlet.GameLocked)
			{
                ...
                // set match length
                AddButton( 200, height - 60, 0xFAE, 0xFAF, 40, GumpButtonType.Reply, 0 );
                AddImageTiled( 310, height - 60, 25, 19, textentrybackground );
                AddTextEntry( 310, height - 60, 25, 25, 0, 40 ,m_gauntlet.MatchLength.TotalMinutes.ToString() );
                AddLabel( 235, height - 60, 0, XmlPoints.GetText(from, 200567) );  // "Length mins: "
			} else
			{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
                case 1:
                    ...
                    state.Mobile.SendGump( new DeathmatchGump( m_gauntlet, state.Mobile, viewpage));
                    break;
                ...
                case 12:
                    ...
                    foreach(DeathmatchGauntlet.ChallengeEntry entry in m_WorkingList)
                    {
                        if(entry != null)
                        {
                    ...
                    state.Mobile.SendGump( new DeathmatchGump( m_gauntlet, state.Mobile, page));
                    break;
                case 13:
                    // page down
                    // try doing the default for non-participants
                ...
                case 300:
                    ...
                    if(m_gauntlet.TargetScore <=0 && m_gauntlet.MatchLength <= TimeSpan.Zero)
                    {
				...
				default:
				{
				    ...
				    if(info.ButtonID >= 4000)
                    {
                        ...
                        {
                            DeathmatchGauntlet.ChallengeEntry entry = (DeathmatchGauntlet.ChallengeEntry)m_WorkingList[selection];
                            ...
                            if(m_gauntlet.Participants != null)
                            {
                                ...
                                foreach(DeathmatchGauntlet.ChallengeEntry masterentry in m_gauntlet.Participants)
                                {
                            ...
                            if(m_gauntlet.Participants != null)
                            {
                                ...
                                if(kickentry != null)
                                {
                                    ...
                                    if(kickentry.Participant != null)
                                    {
                                        ...
                                        kickentry.Participant.SendGump( new DeathmatchGump( m_gauntlet, kickentry.Participant, viewpage));
                                    }
                                }
                            }
		...
		private class MemberTarget : Target
        {
            ...
            public MemberTarget(DeathmatchGauntlet gauntlet, ArrayList list) :  base ( 30, false, TargetFlags.None )
            {
                m_list = list;
                m_gauntlet = gauntlet;
            }
            ...
            protected override void OnTarget( Mobile from, object targeted )
            {
                ...
                if(targeted is Mobile && ((Mobile)targeted).Player)
                {
                    ...
                    if(m_gauntlet.Participants != null)
                    {
                        foreach(DeathmatchGauntlet.ChallengeEntry entry in m_gauntlet.Participants)
                        {
                    ...
                    m_list.Add(new DeathmatchGauntlet.ChallengeEntry(pm));
