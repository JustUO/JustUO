...
namespace Server.Mobiles
{
	public class XmlSpawner : Item, ISpawner
	{
		...
		private static string XmlConfigsDir = "SpawnerConfigs";
							  // default directory for loading .xml config files with LoadConfig

		private const int MaxSmartSectorListSize = 1024;
						  // maximum sector list size for use in smart spawning. This gives a 512x512 tile range.

		private static string defwaypointname; // default waypoint name will get assigned in Initialize
		private const string XmlTableName = "Points";
		private const string XmlDataSetName = "XmlSpawner";

		public static AccessLevel DiskAccessLevel = AccessLevel.Administrator;
								  // minimum access level required by commands that can access the disk such as XmlLoad, XmlSave, and the Save function of XmlEdit
		...
		private static int MaxMoveCheck = 10;
						   // limit number of players that can be checked for triggering in a single OnMovement tick
		#endregion
		...
		private static readonly Hashtable[] GlobalSectorTable = new Hashtable[6];
		#endregion
		...
		private int m_KillReset = defKillReset;
					// number of spawn ticks that pass without kills before killcount gets reset to zero
		...
		private string m_FirstModifiedBy;
		...
		public ArrayList PropertyInfoList = null;
						 // used to optimize property info lookup used by set and get property methods.

		private Hashtable spawnPositionWayTable; // used to optimize #waypoint lookup
		...
		public Point3D MostRecentSpawnPosition { get { return mostRecentSpawnPosition; } set { mostRecentSpawnPosition = value; } }
		...
		public MoonPhase MoonPhase { get { return Clock.GetMoonPhase(Map, Location.X, Location.Y); } }

		public XmlSpawnerGump SpawnerGump { get { return m_SpawnerGump; } set { m_SpawnerGump = value; } }
		...
		*/
		...
		public bool IsInactivated { get { return m_IsInactivated; } set { m_IsInactivated = value; } }
		...
		public bool PlayerCreated { get { return m_PlayerCreated; } set { m_PlayerCreated = value; } }
			...
			set
			{
				if (value.Length > 0)
				{
					...
					else
					{
						...
						if (typestr != null &&
							(BaseXmlSpawner.IsTypeOrItemKeyword(typestr) || typestr.IndexOf("{") != -1 || typestr.StartsWith("*") ||
							 typestr.StartsWith("#")))
						{
							m_SpawnObjects.Add(new SpawnObject(str, 1));
						}
		...
		public int SafeCurrentCount { get { return SafeTotalSpawnedObjects; } }

		public bool FreeRun { get { return m_FreeRun; } set { m_FreeRun = value; } }
		...
		public bool CanFreeSpawn
		{
			get
			{
				...
				if (Running && m_ProximityRange == -1 && (m_ObjectPropertyName == null || m_ObjectPropertyName.Length == 0) &&
					(m_MobPropertyName == null || m_MobPropertyName.Length == 0 || m_MobTriggerName == null ||
		...
		public bool ShowBounds
		{
			...
			set
			{
				if (value && (ShowBounds == false))
				{
					...
					{
						m_ShowBoundsItems = new ArrayList();
					}
					...
					for (int x = 0; x <= m_Width; x++)
					{
						...
						for (int y = 0; y <= m_Height; y++)
						{
							...
							if (NewX == ValidX1 || NewX == ValidX2 || NewX == ValidY1 || NewX == ValidY2 || NewY == ValidX1 ||
								NewY == ValidX2 || NewY == ValidY1 || NewY == ValidY2)
							{
		...
		*/
		...
		[CommandProperty(AccessLevel.Spawner)]
		public int CurrentCount { get { return TotalSpawnedObjects; } }

		[CommandProperty(AccessLevel.Spawner)]
		public WayPoint WayPoint { get { return m_WayPoint; } set { m_WayPoint = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public bool ExternalTriggering { get { return m_ExternalTriggering; } set { m_ExternalTriggering = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public bool ExtTrigState { get { return m_ExternalTrigger; } set { m_ExternalTrigger = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public bool HomeRangeIsRelative { get { return m_HomeRangeIsRelative; } set { m_HomeRangeIsRelative = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public int StackAmount { get { return m_StackAmount; } set { m_StackAmount = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public int KillReset { get { return m_KillReset; } set { m_KillReset = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public double TriggerProbability { get { return m_TriggerProbability; } set { m_TriggerProbability = value; } }

		//added refractory period support
		[CommandProperty(AccessLevel.Spawner)]
		public TimeSpan RefractMin { get { return m_MinRefractory; } set { m_MinRefractory = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public TimeSpan RefractMax { get { return m_MaxRefractory; } set { m_MaxRefractory = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public string TriggerOnCarried { get { return m_ItemTriggerName; } set { m_ItemTriggerName = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public string NoTriggerOnCarried { get { return m_NoItemTriggerName; } set { m_NoItemTriggerName = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public string TriggerObjectProp { get { return m_ObjectPropertyName; } set { m_ObjectPropertyName = value; } }
		...
		public string TriggerObjectName
		{
			get
			{
				if (m_ObjectPropertyItem == null || m_ObjectPropertyItem.Deleted)
				{
					return null;
				}
		...
		[CommandProperty(AccessLevel.Spawner)]
		public Item TriggerObject { get { return m_ObjectPropertyItem; } set { m_ObjectPropertyItem = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public Item SetItem { get { return m_SetPropertyItem; } set { m_SetPropertyItem = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public string MobTriggerProp { get { return m_MobPropertyName; } set { m_MobPropertyName = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public string MobTriggerName { get { return m_MobTriggerName; } set { m_MobTriggerName = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public string PlayerTriggerProp { get { return m_PlayerPropertyName; } set { m_PlayerPropertyName = value; } }

		// time of day activation
		[CommandProperty(AccessLevel.Spawner)]
		public TimeSpan TODStart { get { return m_TODStart; } set { m_TODStart = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public TimeSpan TODEnd { get { return m_TODEnd; } set { m_TODEnd = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public TODModeType TODMode { get { return m_TODMode; } set { m_TODMode = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public TimeSpan DespawnTime { get { return m_DespawnTime; } set { m_DespawnTime = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public int ProximitySound { get { return m_ProximityTriggerSound; } set { m_ProximityTriggerSound = value; } }

		// proximity trigger message parameter
		[CommandProperty(AccessLevel.Spawner)]
		public string ProximityMsg { get { return m_ProximityTriggerMessage; } set { m_ProximityTriggerMessage = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public string SpeechTrigger { get { return m_SpeechTrigger; } set { m_SpeechTrigger = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public bool SpawnOnTrigger { get { return m_SpawnOnTrigger; } set { m_SpawnOnTrigger = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public string GumpState { get { return m_GumpState; } set { m_GumpState = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public int SequentialSpawn { get { return m_SequentialSpawning; } set { m_SequentialSpawning = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public AccessLevel TriggerAccessLevel { get { return m_TriggerAccessLevel; } set { m_TriggerAccessLevel = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public bool AllowGhostTrig { get { return m_AllowGhostTriggering; } set { m_AllowGhostTriggering = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public bool AllowNPCTrig { get { return m_AllowNPCTriggering; } set { m_AllowNPCTriggering = value; } }

		[CommandProperty(AccessLevel.Spawner)]
		public string ConfigFile { get { return m_ConfigFile; } set { m_ConfigFile = value; } }
		...
		[CommandProperty(AccessLevel.Spawner)]
		public Mobile TriggerMob { get { return m_mob_who_triggered; } set { m_mob_who_triggered = value; } }
		...
		{
			if (from == null || from.Deleted || from.AccessLevel < AccessLevel.Spawner ||
				(m_SpawnerGump != null && SomeOneHasGumpOpen))
			{
				return;
			}

			// flag the first person to open the spawner as the placer
			if (FirstModifiedBy == null)
			{
				FirstModifiedBy = from.Name;
			}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (m_SpawnObjects != null)
			{
				for (int i = 0; i < nlist_items && i < m_SpawnObjects.Count; ++i)
				{
					...
					list.Add(
						1060658 + (6 - nlist_items) + i, " {0}\t{1}", typename, ((SpawnObject)m_SpawnObjects[i]).SpawnedObjects.Count);
				}
			}
		...
		public override void OnSingleClick(Mobile from)
		{
			...
			LabelTo(from, Name + (m_Running ? " [On]" : " [Off]"));
		}
					...
					if (ds.Tables[XmlTableName] != null && ds.Tables[XmlTableName].Rows.Count > 0)
					{
						foreach (DataRow dr in ds.Tables[XmlTableName].Rows)
						{
							...
							if (valid_entry)
							{
								if (delayinsec)
								{
									m_MinDelay = TimeSpan.FromSeconds(doubleEntry);
								}
								else
								{
									m_MinDelay = TimeSpan.FromMinutes(doubleEntry);
								}
							}
							...
							if (valid_entry)
							{
								if (delayinsec)
								{
									m_MaxDelay = TimeSpan.FromSeconds(doubleEntry);
								}
								else
								{
									m_MaxDelay = TimeSpan.FromMinutes(doubleEntry);
								}
							}
		...
		public void ReportStatus()
		{
			...
			for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++)
			{
				...
				{
					Console.WriteLine(
						"\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Felucca).Count);
				}
			...
			for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++)
			{
				...
				{
					Console.WriteLine(
						"\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Trammel).Count);
				}
			...
			for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++)
			{
				...
				{
					Console.WriteLine(
						"\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Ilshenar).Count);
				}
			...
			for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++)
			{
				...
				{
					Console.WriteLine(
						"\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Malas).Count);
				}
			...
			for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++)
			{
				...
				{
					Console.WriteLine(
						"\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Tokuno).Count);
				}
		...
		private string setname1 = _traceName[1] = "XmlFind";
		private string setname2 = _traceName[2] = "HasSector";

		private string setname4 = _traceName[4] = "AttachSpeech";
		private string setname5 = _traceName[5] = "HasHold";

		private string setname8 = _traceName[8] = "OnTick";
		private string setname9 = _traceName[9] = "Defrag";
		private string setname10 = _traceName[10] = "Respawn";
		private string setname11 = _traceName[11] = "SetProp";
		private string setname12 = _traceName[12] = "AttachMovement";
		private string setname13 = _traceName[13] = "ActiveSector";

		private string setname15 = _traceName[15] = "DistroTick";
		private string setname16 = _traceName[16] = "GetScaledFaction";
		private string setname17 = _traceName[17] = "FactionOnKill";
		private string setname18 = _traceName[18] = "CheckAcquire";
		...
		private bool AllowTriggering { get { return m_Running && !m_refractActivated && TODInRange && CanSpawn; } }
		...
		public override bool HandlesOnMovement { get { return (m_Running && m_ProximityRange >= 0); } }
		...
		private class MovementTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					if (m_Spawner.m_Running && !m_Spawner.m_proximityActivated && !m_Spawner.m_refractActivated && m_Spawner.TODInRange &&
						m_Spawner.CanSpawn)
					{
						int count = 0;
						int maxspeed = 0;
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register("XmlSpawnerRespawn", AccessLevel.GameMaster, Respawn_OnCommand);
			CommandSystem.Register("XmlSpawnerRespawnAll", AccessLevel.Administrator, RespawnAll_OnCommand);
			...
			CommandSystem.Register("XmlHome", AccessLevel.Spawner, XmlHome_OnCommand);
			...
			CommandSystem.Register("XmlGet", AccessLevel.Spawner, XmlGetValue_OnCommand);
			TargetCommands.Register(new XmlSetCommand());
			//CommandSystem.Register( "XmlSet", AccessLevel.Spawner, new CommandEventHandler( XmlSetValue_OnCommand ) );
			CommandSystem.Register("SmartStat", AccessLevel.Spawner, SmartStat_OnCommand);
			CommandSystem.Register("XmlGo", AccessLevel.Spawner, SpawnEditorGo_OnCommand);
		...
		public class XmlSetCommand : BaseCommand
		{
			...
			{
				AccessLevel = AccessLevel.Spawner;
		...
		private static void LoadDefaults(XmlElement node)
		{
			try
			{
				defProximityRange = int.Parse(node["defProximityRange"].InnerText);
			}
			catch
			{ }
			try
			{
				defTriggerProbability = double.Parse(node["defTriggerProbability"].InnerText);
			}
			catch
			{ }
			try
			{
				defProximityTriggerSound = int.Parse(node["defProximityTriggerSound"].InnerText);
			}
			catch
			{ }
			try
			{
				defMinRefractory = TimeSpan.Parse(node["defMinRefractory"].InnerText);
			}
			catch
			{ }
			try
			{
				defMaxRefractory = TimeSpan.Parse(node["defMaxRefractory"].InnerText);
			}
			catch
			{ }
			try
			{
				defTODStart = TimeSpan.Parse(node["defTODStart"].InnerText);
			}
			catch
			{ }
			try
			{
				defTODEnd = TimeSpan.Parse(node["defTODEnd"].InnerText);
			}
			catch
			{ }
			try
			{
				defAmount = int.Parse(node["defStackAmount"].InnerText);
			}
			catch
			{ }
			try
			{
				defDuration = TimeSpan.Parse(node["defDuration"].InnerText);
			}
			catch
			{ }
			try
			{
				defIsGroup = bool.Parse(node["defIsGroup"].InnerText);
			}
			catch
			{ }
			try
			{
				defTeam = int.Parse(node["defTeam"].InnerText);
			}
			catch
			{ }
			try
			{
				defRelativeHome = bool.Parse(node["defRelativeHome"].InnerText);
			}
			catch
			{ }
			try
			{
				defSpawnRange = int.Parse(node["defSpawnRange"].InnerText);
			}
			catch
			{ }
			try
			{
				defHomeRange = int.Parse(node["defHomeRange"].InnerText);
			}
			catch
			{ }
			try
			{
				defMinDelay = TimeSpan.Parse(node["defMinDelay"].InnerText);
			}
			catch
			{ }
			try
			{
				defMaxDelay = TimeSpan.Parse(node["defMaxDelay"].InnerText);
			}
			catch
			{ }
			int todmode = 0;
			try
			{
				todmode = int.Parse(node["defTODMode"].InnerText);
			}
			catch
			{ }
			switch (todmode)
		...
		public static void XmlUnLoadFromFile(
			string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
		{
			processedmaps = 0;
			processedspawners = 0;
			if (filename == null || filename.Length <= 0)
			...
			else if (Directory.Exists(filename))
			{
				...
				{
					from.SendMessage(
						"UnLoaded a total of {0} .xml files and {2} spawners from directory {1}",
						total_processed_maps,
						filename,
						total_processed_spawners);
				}
		...
		public static void XmlUnLoadFromStream(
			Stream fs, string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
		{
			processedmaps = 0;
			processedspawners = 0;
			...
			if (from != null)
			{
				from.SendMessage(
					string.Format(
						"UnLoading {0} objects{1} from file {2}.",
						"XmlSpawner",
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty),
						filename));
			}
		...
		public static void XmlImportMap(string filename, Mobile from, out int processedmaps, out int processedspawners)
		{
			...
			else if (Directory.Exists(filename))
			{
				...
				from.SendMessage(
					"Imported a total of {0} .map files and {2} spawners from directory {1}",
					total_processed_maps,
					filename,
					total_processed_spawners);
				processedmaps = total_processed_maps;
		...
		private static void ParseNewMapFormat(
			Mobile from,
			string filename,
			string line,
			string[] args,
			int linenumber,
			ref int spawnercount,
			ref int badspawnercount,
			ref int overridemap,
			ref double overridemintime,
		...
		private static void ParseOldMapFormat(
			Mobile from,
			string filename,
			string line,
			string[] args,
							...
							else
							{
								...
								if (group != bool.Parse(GetText(entrynode["GroupSpawn"], "False")))
								{
									...
									try
									{
										...
										{
											op.WriteLine(
												"MSFimport : individual group entry difference: {0} vs {1}",
												GetText(entrynode["GroupSpawn"], "False"),
												group);
										}
									}
									catch
								...
								if (minDelay != TimeSpan.FromSeconds(int.Parse(GetText(entrynode["MinDelay"], "05:00"))))
								{
									...
									try
									{
										...
										{
											op.WriteLine(
												"MSFimport : individual mindelay entry difference: {0} vs {1}",
												GetText(entrynode["MinDelay"], "05:00"),
												minDelay);
										}
									}
									catch
								...
								if (maxDelay != TimeSpan.FromSeconds(int.Parse(GetText(entrynode["MaxDelay"], "10:00"))))
								{
									...
									try
									{
										...
										{
											op.WriteLine(
												"MSFimport : individual maxdelay entry difference: {0} vs {1}",
												GetText(entrynode["MaxDelay"], "10:00"),
												maxDelay);
										}
									}
									catch
									...
									try
									{
										using (StreamWriter op = new StreamWriter("badimport.log", true))
										{
											op.WriteLine(
												...
												GetText(entrynode["WalkRange"], "10"),
												homeRange);
										}
									}
									catch
									...
									try
									{
										using (StreamWriter op = new StreamWriter("badimport.log", true))
										{
											op.WriteLine(
												...
												GetText(entrynode["SpawnRange"], "4"),
												spawnRange);
										}
									}
									catch
		...
		public static void XmlLoadFromFile(
			string filename,
			string SpawnerPrefix,
			Mobile from,
			Point3D fromloc,
			Map frommap,
			bool loadrelative,
			...
			out int processedspawners)
		{
			...
			else if (Directory.Exists(filename))
			{
				...
				{
					from.SendMessage(
						"Loaded a total of {0} .xml files and {2} spawners from directory {1}",
						total_processed_maps,
						filename,
						total_processed_spawners);
				}
		...
		public static void XmlLoadFromFile(
			string filename, string SpawnerPrefix, bool loadnew, out int processedmaps, out int processedspawners)
		{
			processedmaps = 0;
			processedspawners = 0;
		...
		public static void XmlLoadFromStream(
			Stream fs,
			string filename,
			string SpawnerPrefix,
			Mobile from,
			...
			bool verbose)
		{
			...
			if (from != null)
			{
				from.SendMessage(
					string.Format(
						"Loading {0} objects{1} from file {2}.",
						"XmlSpawner",
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty),
						filename));
			}
					...
					foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
					{
						...
						if ((SpawnerPrefix == null) || (SpawnerPrefix.Length == 0) || SpawnName.StartsWith(SpawnerPrefix))
						{
							...
							if (!IsValidMapLocation(SpawnCentreX, SpawnCentreY, SpawnMap))
							{
								...
								{
									from.SendMessage(
										33, "Invalid location '{0}' at [{1} {2}] in {3}", SpawnName, SpawnCentreX, SpawnCentreY, XmlMapName);
								}
								bad_spawner = true;
							}
							...
							if (!bad_spawner)
							{
								foreach (Item i in World.Items.Values)
								{
									...
									if (InContainer && !found_container && (i is Container) && (SpawnCentreX == i.Location.X) &&
										(SpawnCentreY == i.Location.Y) && (SpawnCentreZ == i.Location.Z || SpawnCentreZ == short.MinValue))
									{
							...
							else if (questionable_spawner)
							{
								...
								{
									from.SendMessage(
										33, "Questionable spawner '{0}' at [{1} {2}] in {3}", SpawnName, SpawnCentreX, SpawnCentreY, XmlMapName);
								}
								// log it
								long fileposition = -1;
								try
							...
							if (!bad_spawner)
							{
								...
								XmlSpawner TheSpawn = new XmlSpawner(
									SpawnId,
									SpawnX,
									SpawnY,
									SpawnWidth,
									...
									SpawnDuration,
									SpawnProximityRange,
									SpawnProximityTriggerSound,
									SpawnAmount,
									SpawnTeam,
									SpawnHomeRange,
									SpawnIsRelativeHomeRange,
									Spawns,
									SpawnMinRefractory,
									SpawnMaxRefractory,
									SpawnTODStart,
									SpawnTODEnd,
									SpawnObjectPropertyItem,
									SpawnObjectPropertyName,
									SpawnProximityMessage,
									SpawnItemTriggerName,
									SpawnNoItemTriggerName,
									SpawnSpeechTrigger,
									SpawnMobTriggerName,
									SpawnMobPropertyName,
									SpawnPlayerPropertyName,
									SpawnTriggerProbability,
									SpawnSetPropertyItem,
									SpawnIsGroup,
									SpawnTODMode,
									SpawnKillReset,
									SpawnExternalTriggering,
									SpawnSequentialSpawning,
									SpawnRegionName,
									SpawnAllowGhost,
									SpawnAllowNPC,
									SpawnSpawnOnTrigger,
								...
								{
									from.SendMessage(
										188, "Created '{0}' in {1} at {2}", TheSpawn.Name, TheSpawn.Map.Name, TheSpawn.Location.ToString());
								}
								...
								if (OldSpawner.m_SetPropertyItem == null)
								{
									...
									if (from != null)
									{
										from.SendMessage(
											...
											setObjectName,
											OldSpawner.Name,
											OldSpawner.Location.X,
											OldSpawner.Location.Y,
											OldSpawner.Map);
									}
									// log it
									try
			...
			if (failedobjectitemcount > 0)
			{
				...
				{
					from.SendMessage(
						33, "Failed to initialize TriggerObjects in {0} spawners. Saved to 'badxml.log'", failedobjectitemcount);
				}
			}
			if (failedsetitemcount > 0)
			{
				...
				{
					from.SendMessage(
						33, "Failed to initialize SetItemProperty Objects in {0} spawners. Saved to 'badxml.log'", failedsetitemcount);
				}
			}
			if (badcount > 0)
			{
			...
			if (e.Mobile.AccessLevel >= DiskAccessLevel)
			{
				if (e.Arguments.Length >= 1)
				{
					...
					if (!badargs)
					{
						...
						XmlLoadFromFile(
							filename, SpawnerPrefix, e.Mobile, true, maxrange, false, out processedmaps, out processedspawners);
					}
				}
				else
				{
		...
		private static void SaveSpawns(CommandEventArgs e, bool SaveAllMaps, bool oldformat)
		{
			...
			if (SaveAllMaps)
			{
				e.Mobile.SendMessage(
					string.Format(
						"Saving {0} objects{1} to file {2} from {3}.",
						"XmlSpawner",
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty),
						dirname,
						e.Mobile.Map));
			...
			else
			{
				e.Mobile.SendMessage(
					string.Format(
						"Saving {0} obejcts{1} to file {2} from the entire world.",
						"XmlSpawner",
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty),
						dirname));
			}
		...
		public static bool SaveSpawnList(
			Mobile from, ArrayList savelist, string dirname, Stream stream, bool oldformat, bool verbose)
		{
			...
			foreach (XmlSpawner sp in savelist)
			{
				...
				{
					dr["ObjectPropertyItemName"] = String.Format(
						"{0},{1}", sp.m_ObjectPropertyItem.Name, sp.m_ObjectPropertyItem.GetType().Name);
				}
				else
				{
				...
				{
					dr["SetPropertyItemName"] = String.Format(
						"{0},{1}", sp.m_SetPropertyItem.Name, sp.m_SetPropertyItem.GetType().Name);
				}
				else
				{
		...
		private static void WipeSpawners(CommandEventArgs e, bool WipeAll)
		{
			...
			if (e.Mobile.AccessLevel >= AccessLevel.Administrator)
			{
				...
				{
					e.Mobile.SendMessage(
						"Removing ALL XmlSpawner objects from the world{0}.",
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty));
				}
				else
				{
					e.Mobile.SendMessage(
						"Removing ALL XmlSpawner objects from {0}{1}.",
						e.Mobile.Map,
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty));
				}
		...
		private static void RespawnSpawners(CommandEventArgs e, bool RespawnAll)
		{
			...
			if (e.Mobile.AccessLevel >= AccessLevel.Administrator)
			{
				...
				{
					e.Mobile.SendMessage(
						"Respawning ALL XmlSpawner objects from the world{0}.",
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty));
				}
				else
				{
					e.Mobile.SendMessage(
						"Respawning ALL XmlSpawner objects from {0}{1}.",
						e.Mobile.Map,
						((SpawnerPrefix != null && SpawnerPrefix.Length > 0) ? " beginning with " + SpawnerPrefix : string.Empty));
				}
				...
				foreach (Item i in World.Items.Values)
				{
					...
					{
						Console.WriteLine("Error attempting to add {0}, {1}", i, ex.Message);
					}
				}
		...
		public static void XmlTrace_OnCommand(CommandEventArgs e)
		{
			...
			for (int i = 0; i < MaxTraces; i++)
			{
				if (_traceCount[i] > 0)
				{
					...
					Console.WriteLine(
						"{0} ({4}) {1,21} / {2} calls = {3:####.####} ms/call, {5:p3}",
						i,
						_traceTotal[i],
						_traceCount[i],
						(_traceTotal[i].TotalMilliseconds) / _traceCount[i],
						_traceName[i],
						load);
				}
			}
		...
		public XmlSpawner(
			...
			TimeSpan duration,
			int proximityRange,
			int proximityTriggerSound,
			int amount,
			int team,
			int homeRange,
			bool isRelativeHomeRange,
			SpawnObject[] spawnObjects,
			TimeSpan minRefractory,
			TimeSpan maxRefractory,
			TimeSpan todstart,
			TimeSpan todend,
			Item objectPropertyItem,
			string objectPropertyName,
			string proximityMessage,
			string itemTriggerName,
			string noitemTriggerName,
			string speechTrigger,
			string mobTriggerName,
			string mobPropertyName,
			string playerPropertyName,
			double triggerProbability,
			Item setPropertyItem,
			bool isGroup,
			TODModeType todMode,
			int killReset,
			bool externalTriggering,
			int sequentialSpawning,
			string regionName,
			bool allowghost,
			bool allownpc,
			bool spawnontrigger,
			...
			foreach (SpawnObject so in m_SpawnObjects)
			{
				for (int x = 0; x < so.SpawnedObjects.Count; x++)
				{
					...
					if (o is Item)
					{
						...
						if (DespawnTime.TotalHours > 0 && !item.Deleted && (item.LastMoved < DateTime.UtcNow - DespawnTime) &&
							(item.Parent == Parent) && (!ItemFlags.GetTaken(item) || (item.Parent != null && (item.Parent == Parent))))
							// can despawn if just moved within the same container
						{
							//item.Delete();
							deletelist.Add(item);
							despawned = true;
					...
					else if (o is Mobile)
					{
						...
						* */
						...
						if (DespawnTime.TotalHours > 0 && !m.Deleted && (m.CreationTime < DateTime.UtcNow - DespawnTime) && m.Map != null &&
							m.Map != Map.Internal && !m.Map.GetSector(m.Location).Active)
						{
							//m.Delete();
							deletelist.Add(m);
							despawned = true;
		...
		private int RandomAvailableSpawnIndex(int sgroup)
		{
			...
			for (int i = 0; i < m_SpawnObjects.Count; i++)
			{
				...
				if ((s.MaxCount > s.SpawnedObjects.Count) && (sgroup < 0 || sgroup == s.SubGroup) &&
					...
					(s.SubGroup <= 0 || SubGroupCount(s.SubGroup) + totalcount <= MaxCount))
				{
		...
		public void OnTick()
		{
			...
			{
				m_killcount = 0;
				m_spawncheck = m_KillReset;
					// wait for 1 spawn ticks to pass before resetting.  This can be set to anything you like
			}
		...
		public bool Spawn(bool smartspawn)
		{
			if (m_SpawnObjects != null && m_SpawnObjects.Count > 0 && (m_proximityActivated || CanFreeSpawn) && TODInRange)
			{
				...
				else
				{
					...
					if (Spawn(SpawnIndex, smartspawn, sobj.SpawnsPerTick))
					{
						if (!smartspawn)
						{
		...
		public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord, bool ignoreloopprotection)
		{
			...
			if ((map == null) || (map == Map.Internal) || (m_SpawnObjects == null) || (m_SpawnObjects.Count == 0) || (index < 0) ||
			...
			if (TheSpawn != null)
			{
							...
							switch (keyvalueargs[0])
							{
								...
								case "#CONDITION":
									...
									if (ckeyvalueargs.Length > 1)
									{
										...
										if (!BaseXmlSpawner.CheckPropertyString(this, this, ckeyvalueargs[1], m_mob_who_triggered, out status_str))
										{
											return false;
										}
									}
				...
				if (BaseXmlSpawner.IsTypeOrItemKeyword(typeName))
				{
					...
					bool completedtypespawn = BaseXmlSpawner.SpawnTypeKeyword(
						this,
						TheSpawn,
						typeName,
						substitutedtypeName,
							...
							if (o is Mobile)
							{
								...
								*/
								...
								BaseXmlSpawner.ApplyObjectStringProperties(
									this, substitutedtypeName, m, m_mob_who_triggered, this, out status_str);
		...
		public bool HasSubGroups()
		{
			...
			for (int j = 0; j < m_SpawnObjects.Count; j++)
			{
				if (((SpawnObject)m_SpawnObjects[j]).SubGroup > 0)
				{
					return true;
				}
			}
				...
				if (wayargs != null && wayargs.Length > 0)
				{
					...
					if (wayargs[0] == "SERIAL")
					{
						...
						if (wayargs.Length > 1)
						{
							...
							try
							{
								sernum = (int)Convert.ToUInt64(wayargs[1].Substring(2), 16);
							}
							catch
							{ }
		...
		public bool CanFit(
			int x, int y, int z, int height, bool checkBlocksFit, bool checkMobiles, bool requireSurface, Mobile mob)
		{
			Map map = Map;
			...
			{
				Console.WriteLine(
					"landtile at {0},{1},{2} wet={3} impassable={4} hassurface={5}", x, y, z, wet, impassable, hasSurface);
			}
			...
			if (checkMobiles)
			{
				for (int i = 0; i < mobs.Count; ++i)
				{
					...
					if (m.Location.X == x && m.Location.Y == y && (m.IsPlayer() || !m.Hidden))
					{
		...
		private void FindTileLocations(
			...
			int width,
			int height,
			List<int> includetilelist,
			List<int> excludetilelist,
			TileFlag tileflag)
		{
		...
		private void FindRegionTileLocations(
			ref List<Point3D> locations, Region r, List<int> includetilelist, List<int> excludetilelist, TileFlag tileflag)
		{
		...
		public Point3D GetSpawnPosition(
			bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning)
		{
			...
			bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning, Mobile mob)
		{
			...
			if (includetilelist != null || excludetilelist != null || tileflag != TileFlag.None)
			{
				...
				{
					FindTileLocations(ref locations, Map, m_X, m_Y, m_Width, m_Height, includetilelist, excludetilelist, tileflag);
				}
			}
					...
					if (delete_this_entry)
					{
						...
						if (from != null)
						{
							CommandLogging.WriteLine(
								from,
								"{0} {1} removed from XmlSpawner {2} '{3}' [{4}, {5}] ({6}) : {7}",
								from.AccessLevel,
		...
		internal string GetSerializedObjectList2()
		{
			...
			foreach (SpawnObject so in m_SpawnObjects)
			{
				...
				sb.AppendFormat(
					"{0}:MX={1}:SB={2}:RT={3}:TO={4}:KL={5}:RK={6}:CA={7}:DN={8}:DX={9}:SP={10}:PR={11}",
					so.TypeName,
					so.ActualMaxCount,
					so.SubGroup,
					so.SequentialResetTime,
					so.SequentialResetTo,
					so.KillsNeeded,
					so.RestrictKillsToSubgroup ? 1 : 0,
					so.ClearOnAdvance ? 1 : 0,
					so.MinDelay,
					so.MaxDelay,
					so.SpawnsPerTick,
					so.PackRange);
			...
			public SpawnObject(Mobile from, XmlSpawner spawner, string name, int maxamount)
			{
				if (from != null && spawner != null)
				{
					...
					if (!found)
					{
						CommandLogging.WriteLine(
							from,
							"{0} {1} added to XmlSpawner {2} '{3}' [{4}, {5}] ({6}) : {7}",
							from.AccessLevel,
							CommandLogging.Format(from),
							spawner.Serial,
							spawner.Name,
							spawner.GetWorldLocation().X,
							spawner.GetWorldLocation().Y,
							spawner.Map,
							name);
					}
				}
