...
namespace Server
{
    public partial class Ability
    {
        ...
        public class FireField : Item
        {
            ...
            private readonly Point3D m_MoveToPoint;
                ...
                : base(GetItemID(south))
            {
                ...
                m_MinDamage = min;
                m_MaxDamage = max;
                m_Destroy = DateTime.UtcNow + TimeSpan.FromSeconds(duration + 1.5);
                m_MoveToPoint = point;
                m_MoveToMap = map;
                m_List = new List<Mobile>();
                m_Timer = Timer.DelayCall(TimeSpan.Zero, TimeSpan.FromSeconds(1), OnTick);
                Timer.DelayCall(TimeSpan.Zero, TimeSpan.FromSeconds(1.5), Move);
            }
            ...
            private void OnTick()
            {
                ...
                else if (m_MinDamage != 0)
                {
                    foreach (var m in GetMobilesInRange(0))
                    {
                        ...
                        if (m_Owner != null)
                        {
                            if (CanTarget(m_Owner, m, true, true, false))
                                m_List.Add(m);
                        }
                        else
                            m_List.Add(m);
            ...
            {
                if (m != m_Owner)
                    AOS.Damage(m, (m_Owner == null) ? m : m_Owner, Utility.RandomMinMax(m_MinDamage, m_MaxDamage), 0,
                        100, 0, 0, 0);
            }
                        ...
                        for (var j = -2; j < 5; j++)
                            if ((i == -2 || i == 2) || (j == -2 || j == 2))
                                Effects.SendMovingParticles(
                                    new Entity(Serial.Zero, new Point3D(m_From.X + i, m_From.Y + j, m_From.Z + 14),
                                        m_From.Map),
        ...
        public static void SoulDrain(Mobile from)
        {
            ...
            foreach (var m in from.GetMobilesInRange(8))
                if (m != null)
        ...
        internal class FlameWaveTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                for (var i = -m_Count; i < m_Count + 1; i++)
                {
                    for (var j = -m_Count; j < m_Count + 1; j++)
                    {
                        ...
                        {
                            Effects.SendLocationParticles(
                                EffectItem.Create(m_Point, m_Map, EffectItem.DefaultDuration), 0x3709, 10, 30, 5052);
                        }
                    }
                }
            ...
            private void SetupDamage(Mobile from)
            {
                foreach (var m in from.GetMobilesInRange(10))
                {
                    ...
                    {
                        DelayCall(TimeSpan.FromMilliseconds(300*(GetDist(m_StartingLocation, m.Location)/3)),
                            new TimerStateCallback(Hurt), m);
                    }
                }
            }
        ...
        {
            public OtherFireFieldItem(int itemID, Point3D loc, Mobile caster, Map map, TimeSpan duration, int val,
                int damage)
        ...
        {
            var number = (int) d + Utility.RandomMinMax(-1, 1);
        ...
        public class JaggedLineTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                var p = new Point3D(m_Point.X, m_Point.Y, m_Map.GetAverageZ(m_Point.X, m_Point.Y));

                if (m_Map.CanFit(p, 16, false, false))
                {
                    var canplace = true;

                    foreach (var item in m_Map.GetItemsInRange(p, 0))
                    {
                        if (item != null)
                        {
...
{
    public class TheSixCommands
    {
        public static void Initialize()
        {
            CommandSystem.Register("SimpleFlame", AccessLevel.Seer, SimpleFlame_OnCommand);
            CommandSystem.Register("SoulDrain", AccessLevel.Seer, SoulDrain_OnCommand);
            CommandSystem.Register("FlameWave", AccessLevel.Seer, FlameWave_OnCommand);
            CommandSystem.Register("FlameCross", AccessLevel.Seer, FlameCross_OnCommand);
            CommandSystem.Register("CrimsonMeteor", AccessLevel.Seer, CrimsonMeteor_OnCommand);
            CommandSystem.Register("JaggedFire", AccessLevel.Seer, JaggedFire_OnCommand);
        }

        [Description("Use the Six's Simple Flame attack")]
        public static void SimpleFlame_OnCommand(CommandEventArgs e)
        {
            e.Mobile.BeginTarget(10, false, TargetFlags.Harmful, SimpleFlame_CallBack);
        }
