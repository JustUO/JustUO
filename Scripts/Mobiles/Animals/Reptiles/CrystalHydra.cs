...
namespace Server.Mobiles
{
    ...
    public class CrystalHydra : BaseCreature
    {
        ...
        public override void BreathStart(Mobile target)
        {
            ...
            foreach (var m in GetMobilesInRange(BreathRange))
            {
                if (count++ > 3)
                    break;

                if (m != null && m != target && m.Alive && !m.IsDeadBondedPet && CanBeHarmful(m) && m.Map == Map &&
                    !IsDeadBondedPet && m.InRange(this, BreathRange) && InLOS(m) && !BardPacified)
                    Timer.DelayCall(TimeSpan.FromSeconds(BreathEffectDelay),
                        new TimerStateCallback(BreathEffect_Callback), m);
            }
            Timer.DelayCall(TimeSpan.FromSeconds(BreathEffectDelay), new TimerStateCallback(BreathEffect_Callback),
                target);
        }
