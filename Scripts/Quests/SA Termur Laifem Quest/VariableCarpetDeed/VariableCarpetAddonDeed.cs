...
namespace Server.Gumps
{
    public class VariableCarpetGump : Gump
    {
        ...
        public VariableCarpetGump(BaseAddonDeed deed, IPoint3D p, Map map, int width, int height) : base(30, 30)
        {
            ...
            for (int i = 0, index = 0; i < VariableCarpetInfo.Infos.Length; ++i, ++index)
            {
                ...
                var info = VariableCarpetInfo.GetInfo(i);

                for (var j = 0; j < info.Entries.Length; ++j)
                {
                    if (info.Entries[j].OffsetX >= 0 && info.Entries[j].OffsetY >= 0)
                        AddItem(20 + (index*140) + info.Entries[j].OffsetX, 46 + info.Entries[j].OffsetY,
                            info.Entries[j].ItemID);
                }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (info.ButtonID >= 1)
            {
                ...
                var res = addon.CouldFit(m_P3D, m_Map, from, ref house);
    ...
    public class VariableCarpetPlacedGump : Gump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            {
                case 0:
                    //Case uses the ActionIDs defined above. Case 0 defines the actions for the button with the action id 0 
                {
                    //Cancel 
