...
namespace Server
{
	...
	public static class RangeExtUtility
	{
		...
		///     'floor' is used as a negative offset for the current Z-axis being checked.
		///     'roof' is used as a positive offset for the current Z-axis being checked.
		/// </summary>
		...
		public static List<ISpawner> GetSpawnersInRange(this IPoint3D center, Map map, int range)
		{
			...
			var list = ipe.OfType<Item>().Where(o => InRange2D(center, o, range)).OfType<ISpawner>().ToList();
		...
		public static List<T> GetEntitiesInRange<T>(this IPoint3D center, Map map, int range) where T : IEntity
		{
			...
			var ipe = map.GetObjectsInRange(center.ToPoint3D(), range);
			var list = ipe.OfType<T>().Where(o => InRange2D(center, o, range)).ToList();
		...
		public static Point3D[] GetAllPointsInRange(this IPoint3D center, Map map, int min, int max, bool avgZ = true)
		{
			...
			var points = new List<Point3D>((int)Math.Ceiling((Math.PI * Math.Sqrt(max)) - (Math.PI * Math.Sqrt(min))));
