...
namespace VitaNex.Items
{
	...
	{
		private static List<Mobile> Cache { get { return new List<Mobile>(Stinky.Keys); } }
		...
		public static void DoStinkEffect(Mobile m)
		{
			...
			new PoisonExplodeEffect(
				...
				m.Map,
				1,
				effectHandler: e => e.Source.GetMobilesInRange(e.Map, 0).ForEach(
					m1 =>
					{
						if (m1 != null && !m1.Deleted && m1 != m && m1.Body.IsHuman && !m1.Hidden && m1.Alive)
						{
							Effects.PlaySound(m1.Location, m1.Map, Utility.RandomList(1065, 1066, 1067));
						}
					})).Send();
		}
		...
		public virtual void ExplosionStink(EffectInfo info)
		{
			...
			Timer.DelayCall(
				...
				{
					foreach (Mobile m in
						info.Source.GetMobilesInRange(info.Map, 0)
							.Where(m => m != null && !m.Deleted && !m.Hidden && (m.Alive || AllowDeadTarget)))
					{
