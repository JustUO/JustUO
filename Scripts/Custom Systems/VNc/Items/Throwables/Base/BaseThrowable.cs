...
namespace VitaNex.Items
{
		...
		where TEntity : IEntity
	{
		...
		public virtual bool CanThrow(Mobile m, bool message)
		{
			...
			{
				if (m.FindItemOnLayer(Layer) != this)
				{
					if (message)
					{
						m.SendMessage(37, "Put the {0} in your backpack to start throwing!", Name);
					}
			...
			if (!m.Alive && !AllowDeadUser)
			{
				if (message)
				{
					m.SendMessage(37, "You can't throw the {0} while dead.", Name);
				}
			...
			if (m.AccessLevel < AccessLevel.GameMaster)
			{
				if (RequiredSkillValue > 0 && m.Skills[RequiredSkill].Value < RequiredSkillValue)
				{
					if (message)
					{
						m.SendMessage(37, "You do not have the skill required to throw the {0}.", Name);
					}
				...
				if (!m.CanBeginAction(GetType()))
				{
					if (message)
					{
						m.SendMessage(37, "Your arms are tired, you must wait to throw another {0}!", Name);
					}
				...
				if (!AllowCombat && SpellHelper.CheckCombat(m))
				{
					if (message)
					{
						m.SendMessage(37, "You can't throw the {0} while in combat!", Name);
					}
				...
				if (m.BodyMod > 0 && !m.BodyMod.IsHuman)
				{
					if (message)
					{
						m.SendMessage(37, "How are you supposed to throw the {0} with no hands, beast?", Name);
					}
		...
		public virtual bool CanThrowAt(Mobile m, TEntity target, bool message)
		{
			...
			if (m == target as Mobile)
			{
				if (message)
				{
					m.SendMessage(37, "You can't throw the {0} at yourself!", Name);
				}
		...
		protected virtual void OnBeginTarget(Mobile m, bool message)
		{
			...
			m.LocalOverheadMessage(MessageType.Emote, 0x55, true, String.Format("You begin to swing the {0}...", Name));
			m.NonlocalOverheadMessage(
				MessageType.Emote, 0x55, true, String.Format("{0} begins to swing the {1}...", m.RawName, Name));
		}
		...
		protected virtual void OnThrownAt(Mobile m, TEntity target)
		{
			...
			if (ThrowRecovery > TimeSpan.Zero)
			{
				if (UpdateTimer == null)
				{
					UpdateTimer = PollTimer.CreateInstance(
