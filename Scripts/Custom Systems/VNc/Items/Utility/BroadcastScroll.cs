...
namespace VitaNex.Items
{
	...
	public class BroadcastScroll : Item, IUsesRemaining
	{
		...
		public void Broadcast(Mobile from)
		{
			...
			if (reach - staff > 0)
			{
				from.SendMessage(0x55, "Your broadcast was seen by {0} people!", reach.ToString("#,#"));
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			{
				SuperGump.Send(new BroadcastScrollGump(from as PlayerMobile, scroll: this));
			}
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (ShowUsesRemaining)
			{
				if (!Stackable)
				{
					...
					list.Add(1060584, String.Format("{0}", UsesRemaining));
				}
				else
				{
					list.Add("uses remaining: {0} (total)", UsesRemaining * Amount);
				}
			}

			list.Add("<basefont color=#{0:X6}>Use: Broadcast A Global Message<basefont color=#ffffff>", Color.SkyBlue.ToArgb());
		}
