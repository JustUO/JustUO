...
namespace VitaNex.SuperGumps.UI
{
	...
	{
		private static readonly Type _TypeOfSkillName = typeof(SkillName);
		...
		protected override void CompileList(List<SkillName> list)
		{
			...
			foreach(
				SkillName skill in Enum.GetValues(_TypeOfSkillName).Cast<SkillName>().Where(skill => !IgnoredSkills.Contains(skill))
				)
			{
				List.Add(skill);
			}
