	    ...
	    private class TAFGump : Gump
		{
			...
			public TAFGump(Mobile from, string initialText = "") : base(30, 20)
			{
				...
				AddHtml(170, 20, 315, 165,
					String.Format("Welcome to the {0} Referral System!  Click the button below to target the character that invited you to play, and you will both receive a referral point to spend in the Referral Rewards Shop.  You cannot set your own accounts as your referrer.  You can access the shop by saying [referralrewards.  You can only purchase one referral gift per day.  For every player that marks you as their referrer, you will receive another Referral Point.  Thank you for playing {1}!", TAFShardName, TAFShardName),
					true, true);
				...
				if (target is PlayerMobile)
				{
					...
					else
					{
					    ...
					    if (uniqueIP)
						{
                            if (pm != null)
                            {
                                if (pm.ReferredList.Any(tafip_List.Contains))
                                {
							...
							if (!toldu)
							{
							    ...
							    if (pm != null)
							    {
							        ...
							        {
                                        pm.ReferredList.Add(ip);
							        }
							    }

							    XmlReferralRewards re = (XmlReferralRewards) XmlAttach.FindAttachment(friend, typeof (XmlReferralRewards));
								XmlReferralRewards me = (XmlReferralRewards) XmlAttach.FindAttachment(from, typeof (XmlReferralRewards));
								...
								{
									friend.SendMessage(39, "You are not eligable for referral rewards on {0}.", TAFShardName);
								}
